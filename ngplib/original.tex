...
Describe and define how ``Encryption'' is done, which bytes are
encrypted (not header + crc iow. The parameterblock, all after
req\_ver is best I think (status message not in plaintext).


% Add Search location, Region and Overview, type definition.\\ DONE 20040611

% Add Search Item type, and sub-type, definition.\\ \\ DONE 20040610

% XXX: Sub-type for Search Item?\\ Is poi-type 20040611


% Document Message Reply's different buffer contents, like MMS and SMS.\\ DONE 20040615

% ObjectType has duplicates. Like Route <-> Itinerary. \\ \\ DONE 20040611

% XXX: Send me all checksums param. From client to server.\\
% Done 20040608 specified as a own request.


% Document what a Parameter block is. \\ Done 20040609

% Document different types of parameters, string, uint32 etc.\\
% Done 20040609


% Add top region list definition. DONE 20040611 \\


% Default-values for parameter values, client doesn't need to send parameter if
% server has defined default and client doesn't want other value.\\ \\ DONE 20040611

% On SERVER\_OVERLOADED and SERVER\_UPGRADE\_IN\_PROGRESS time to retry param
% from server to client.\\ DONE 20040615

%Route using favorite, identify that it is a favorite as orig/dest. 
% Statistics.\\ DONE 20040615 Orig/Dest fav coord params


% Some way to version top-region, (fill in here), with/without GMS-cell largest
% cities.\\
% Done 20040608 as own request.

% Change routeID to string.\\ DONE 20040611

%New param User UIN that is UIN and User login is login and nothing else. XXX: Perhaps a User ID that is backward compatible UIN/login. \\ DONE 20040611

% Describe Matrix Of Doom, when things are done and that WFST is sent in reply.\\ MOD_II included. DONE 20040615

% Describe which params that can be used together to authenticate user.\\ DONE 20040615


\section{Future things}
\begin{itemize}
  
\item Route data: In this specification. Add new functionality
  Roadsigns, (Landmarks done).
  
\item XXX: Auth-cookie that is for the socket connection (In http).
  For Keep-Alive. The authed user is stored in the Http-Connection
  with the Auth-cookie and Keep-Alive http-requests uses this to be
  faster. Or perhaps authentication caching in server. THIS IS LOW
  PRIO, server caches user-login and -data.
     
   \item Search using GSM-Cell-position track as country,area. A list
     of last visited cells in a byte-array-parameter.

\item Search expand as in martin's and pi's patent.
  
\item GSM cell data information in top region list. A ``Country-ID''
  plus a list of ``region-ID''.

\item XXX: Cities list of large cities per top region.
\end{itemize}


\section{Questions}
\begin{itemize}
\item[XXX:]  Change route data format from clever to clean.

\item[XXX:] Encryption: 
  \begin{enumerate}
  \item SSL(TLS)
    \begin{description}
    \item[+] Safe, using certificate to verify server (and client).
    \item[-] An extra SSL-handshake, cpu-intense. Might not be
      available on all platforms.
    \end{description}
  \item Magic XOR as before. 
    \begin{description}
    \item[+] Fast, easy to implement,   
    \item[-] no real security, just obscurity.
    \end{description}
  \end{enumerate}

% \item[XXX:] Compression? Compression added to parameterblock from protoVer 0xc

% \item[XXX:] Http encapsulation. Can provide compression. Https can
%   provide encryption.
% Works since first tme client about 2003.

  
\item[XXX:] Reorder and reuse types in RequestType enum?

\item[XXX:] Other coordinate format? MC2! Routes?
...
~\\ XXX: Needs type, req\_ver (protover)?
...
\begin{description}
  ...
  \item[{\tt ROUTE\_REPLY\_NO\_ROUTE\_CHANGE}] When an old route id and
    a reroute reason has benn supplied in the request, the server may
...
\subsection{NAV\_GPS\_ADDRESS\_REQ v.1}

A request for a conversion from position to address, sent
from client to server.

\begin{list}{\textbf{ParamID}}{}
\item \textbf{1400} \textbf{Type} int32,int32 \textbf{Name} GPS Curr pos
                 \textbf{Default~value} - \\
  \index{GPS Curr pos, Parameter}
  \index{1400, ParamID}
  \label{GPS Curr pos}
  \textbf{Description} Current coordinate.
\item \textbf{1401} \textbf{Type} uint16 \textbf{Name} GPS Heading
                 \textbf{Default~value} - \\
  \index{GPS Heading, Parameter}
  \index{1401, ParamID}
  \label{GPS Heading}
  \textbf{Description} Current heading.
\end{list}


\subsection{NAV\_GPS\_ADDRESS\_REPLY v.1}

A reply to a gps\_address\_request, sent from the server
to the client.
\begin{list}{\textbf{ParamID}}{}
\item \textbf{1500} \textbf{Type} string \textbf{Name} TopRegionName
                 \textbf{Default~value} - \\
  \index{TopRegionName, Parameter}
  \index{1500, ParamID}
  \label{TopRegionName}
  \textbf{Description} Name of the top region.
...
\item \textbf{1505} \textbf{Type} int32,int32 \textbf{Name} GPS Pos
                 \textbf{Default~value} - \\
  \index{GPS Pos, Parameter}
  \index{1505, ParamID}
  \label{GPS Pos}
  \textbf{Description} Position coordinate.
\end{list}
...
\section{Size comparison with protover 7}

This appendix contains a small comparison of the size of a typical
search request and reply in protocol version 7 and 8. Version 8
introduced the parametric format. This comparison was made to see if
the overhead of the parameters affected the amount of transmitted data
in an adverse way. It was concluded that it did not, so the parametric
format was introduced. This comparison is now mostly of historical
interest.

\subsection{Search request size}

\subsubsection{Protocol version 8}

\begin{tabular}{|l|l|l|}
\hline
Param           & Size             & Comment          \\\hline
Packet header   & 14               & Header of packet \\\hline
Start index     & 0                & 0,0 Default      \\\hline
City            & 4+City+1         & ``Södra sandby'' \\\hline
item            & 4+item+1         & ``Vattug''       \\\hline
TopRegionID     & 4+4              & 1                \\\hline
Max Nbr Matches & 0                & 10 Default       \\\hline
                &                  &                  \\\hline
Total size      & City + item + 32 &                  \\\hline
Ex              & 12 + 6 +32 = 50  &                  \\\hline
\end{tabular}


\subsubsection{Protocol version 7}

\begin{tabular}{|l|l|l|}
\hline
Param         & Size              & Comment          \\\hline
Packet header & 13                & Header of packet \\\hline
Start index   & 2                 &                  \\\hline
City          & City+1            & ``Södra sandby'' \\\hline
areaID        & 1                 &                  \\\hline
Destination   & item+1            & ``Vattug''       \\\hline
Country       & 4                 & 1                \\\hline
Lat           & 4                 &                  \\\hline
Lon           & 4                 &                  \\\hline
Heading       & 1                 &                  \\\hline
              &                   &                  \\\hline
Total size    & City + item + 31  &                  \\\hline
Ex            & $12 + 6 +31 = 49$ &                  \\\hline
\end{tabular}

\subsubsection{Result}
Protocol version 8 is $50/49 = 1.020 = 2\%$ bigger.
...
% <auth indentingandlinebreaks="false" >
% <auth_user>iphpuser</auth_user>
% <auth_passwd>password</auth_passwd>
% </auth>

% <search_request transaction_id="R">
% <search_request_header 
% position_search_items="true"
% position_search_areas="true"
% search_area_starting_index="0"
% search_area_ending_index="9"
% search_item_starting_index="0"
% search_item_ending_index="9"
% full_search_area_match_purge="false">

% <search_preferences>
% <user_id>idaniel</user_id>
% </search_preferences>
...
% <search_area_query>Södra sandby</search_area_query>
% <search_item_query>Vattug</search_item_query>
% </search_query>
...
% Total XML size: 715 (+35 newlines). Incl. login.
...
% Total HuffmanXML size: 203 (+35 newlines) Incl. login.\\


\subsection{Search reply size}
...
\hline
Param          & Size                 & Comment             \\\hline
Packet header  & 15                   & Header of packet    \\\hline
Area nbrs      & 4+2+2                & 10,16               \\\hline
Overview match & 4 + 10 * ( 2 + strlen( ``a:44:1:: `` ) + & \\
               & strlen( "Lund" ) + 1 ) = 164             & \\\hline
               &                                          & \\\hline
Total size     & 15 + 8 + 164 = 187                       & \\\hline
\end{tabular}
...
Param            & Size     & Comment                        \\\hline
Packet header    & 14       & Header of packet               \\\hline
Search list type & 1        & 1                              \\\hline
nbr matches      & 2        & 10                             \\\hline
Matches starting index & 2  & 0                              \\\hline
Total matches    & 2        & 10                             \\\hline
Number regions   & 2        & 0                              \\\hline
Regions          & 0        &                                \\\hline
Match            & 10* ( 4 + strlen( ``a:44:1:: `` ) +     & \\
                 &       strlen( "Lund" ) + 1 + 4 ) =  180 & \\\hline
                 &     &                                     \\\hline
Total size       & 14 + 1 + 4*2 + 180 = 203  &               \\\hline
\end{tabular}


\subsubsection{Result}

If {\tt sizeof(SearchLocationType)} is the same protocol version 8 is 
$207/203 = 1.020 = 2\%$ more, otherwise $187/203 = 0.921 = 8\%$ less
